...
export const api = {
  ...
  async createRestockingOrder(budget, warehouse = null) {
    const body = { budget }
    if (warehouse && warehouse !== 'all') body.warehouse = warehouse
    const response = await axios.post(`${API_BASE_URL}/restocking/order`, body)
    return response.data
  }
}
